...
setwd(inDir)
load("sample_overlaps_per_million_reads.Rdata")
...
#split sample_overlaps_phenotype_sorted into two halves to better fit on plot:
row_number=(nrow(sample_overlaps_phenotype_sorted))
###find out way to do this with any dataset by row:
sample_overlaps_phenotype_sorted_a = sample_overlaps_phenotype_sorted[1:1566, ]
sample_overlaps_phenotype_sorted_b = sample_overlaps_phenotype_sorted[1567:3074, ]
###
...
#pdf("/home/jamtor/projects/neura/results/stanley_pc.overlaps/sample_overlaps_boxplot_fpkm_phenotype_sorted.pdf", height = 14, width = 20)
sample_overlaps_boxplot = ggplot(sample_overlaps_phenotype_sorted_a, aes(x=reorder(Transcript_ID, -Median), y=Overlap_count_per_million_reads)
...
#pdf("/home/jamtor/projects/neura/results/stanley_pc.overlaps/sample_overlaps_boxplot_fpkm_phenotype_sorted.pdf", height = 14, width = 20)
sample_overlaps_boxplot = ggplot(sample_overlaps_phenotype_sorted_b, aes(x=reorder(Transcript_ID, -Median), y=Overlap_count_per_million_reads)
) + geom_point(aes(colour=phenotype, fill=phenotype), position = position_jitterdodge(jitter.width = 1.0, jitter.height = 0, dodge.width=0.75
)) + scale_colour_manual(values = c("firebrick2", "dodgerblue3", "chartreuse3", "orange")
) + geom_boxplot (aes(fill=phenotype)
                  #) + scale_y_log10(
