...
transcript_ids = as.vector(unique(sample_overlaps_a$Transcript_ID))
i=1
...
for (t_id in transcript_ids) {
  ...
  writeLines("\n")
  subset = subset(sample_overlaps_a, Transcript_ID == t_id)
...
#merge 'zero_counts_df' and 'sample_overlaps_a' dataframes by Transcript_ID:
sample_overlaps_a = merge(sample_overlaps_a, zero_counts_df, by="Transcript_ID")
...
sample_overlaps_age_sorted_a=merge(sample_overlaps_a, patient_metadata_age_sorted, by="Sample_name")
sample_overlaps_age_sorted_a=sample_overlaps_age_sorted_a[order(sample_overlaps_age_sorted_a$age),]
###find a better way to do the following:
sample_overlaps_age_sorted_a[1:522,8] = "25-40"
sample_overlaps_age_sorted_a[523:1276,8] = "41-55"
sample_overlaps_age_sorted_a[1276:1537,8] = "56-70"
colnames(sample_overlaps_age_sorted_a) = c("Sample_name", "Transcript_ID", "Overlap_count_per_million_reads", "Median", "Zero_counts_count", "sample", "age", "age_group")
###

#create boxplot of sample overlap counts with Transcript_IDs on x axis, order sorted by median for each transcript:
pdf(paste0(inDir, "/pfc_sample_overlaps_boxplot_fpkm_byage_a.pdf.pdf"), height = 18, width = 34)
sample_overlaps_boxplot_a = ggplot(sample_overlaps_age_sorted_a, aes(x=reorder(Transcript_ID, -Median), y=Overlap_count_per_million_reads)
