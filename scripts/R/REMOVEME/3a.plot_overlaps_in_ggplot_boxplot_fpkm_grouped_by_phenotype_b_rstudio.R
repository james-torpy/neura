...
sample_overlaps_phenotype_sorted_a=merge(sample_overlaps_a, patient_metadata_phenotype_sorted, by="Sample_name")

#create boxplot of sample overlap counts with Transcript_IDs on x axis, order sorted by median for each transcript:
#pdf("/home/jamtor/projects/neura/results/stanley_pc.overlaps/sample_overlaps_boxplot_fpkm_phenotype_sorted_a.pdf", height = 14, width = 20)
sample_overlaps_boxplot_a = ggplot(sample_overlaps_phenotype_sorted_a, aes(x=reorder(Transcript_ID, -Median), y=Overlap_count_per_million_reads)
