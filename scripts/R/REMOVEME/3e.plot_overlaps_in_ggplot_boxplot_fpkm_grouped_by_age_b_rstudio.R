...
sample_overlaps_age_sorted_b=merge(sample_overlaps_b, patient_metadata_age_sorted, by="Sample_name")
sample_overlaps_age_sorted_b=sample_overlaps_age_sorted_b[order(sample_overlaps_age_sorted_b$age),]
###find a better way to do the following:
sample_overlaps_age_sorted_b[1:522,8] = "25-40"
sample_overlaps_age_sorted_b[523:1276,8] = "41-55"
sample_overlaps_age_sorted_b[1276:1537,8] = "56-70"
colnames(sample_overlaps_age_sorted_b) = c("Sample_name", "Transcript_ID", "Overlap_count_per_million_reads", "Median", "Zero_counts_count", "sample", "age", "age_group")
###

#create boxplot of sample overlap counts with Transcript_IDs on x axis, order sorted by median for each transcript:
pdf(paste0(inDir, "/pfc_sample_overlaps_boxplot_fpkm_byage_b.pdf"), height = 18, width = 34)
sample_overlaps_boxplot_b = ggplot(sample_overlaps_age_sorted_b, aes(x=reorder(Transcript_ID, -Median), y=Overlap_count_per_million_reads)
