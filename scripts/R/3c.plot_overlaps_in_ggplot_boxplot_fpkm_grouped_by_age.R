...
sample_overlap = merge(sample_overlaps, zero_counts_df, by="Transcript_ID")
...
sample_overlaps_age_sorted=merge(sample_overlaps, patient_metadata_age_sorted, by="Sample_name")
sample_overlaps_age_sorted=sample_overlaps_age_sorted[order(sample_overlaps_age_sorted$age),]
###find a better way to do the following:
sample_overlaps_age_sorted[1:2,8] = "fetal"
sample_overlaps_age_sorted[3:524,8] = "25-40"
sample_overlaps_age_sorted[525:1278,8] = "41-55"
sample_overlaps_age_sorted[1279:1539,8] = "56-70"
colnames(sample_overlaps_age_sorted) = c("Sample_name", "Transcript_ID", "Overlap_count_per_million_reads", "Median", "Zero_counts_count", "sample", "age", "age_group")
###

#create boxplot of sample overlap counts with Transcript_IDs on x axis, order sorted by median for each transcript:
pdf(paste0(inDir, "/pfc_sample_overlaps_boxplot_fpkm_byage,", transcripts_group_suffix, ".pdf"), height = 18, width = 34)
sample_overlaps_boxplot = ggplot(sample_overlaps_age_sorted, aes(x=reorder(Transcript_ID, -Median), y=Overlap_count_per_million_reads)
